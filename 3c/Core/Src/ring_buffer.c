...
//@param data: the pointer to the adress to write the data
uint8_t ring_buffer_get(ring_buffer_t *ring_buffer,uint8_t *data)
{
	if(ring_buffer_is_empty(ring_buffer) == 0) { //checks directly with the function bellow
...
//@brief this returns the size of the ring buffer
